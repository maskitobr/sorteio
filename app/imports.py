...
FILES_PATH = f"{BASEDIR}/static/files/"
...
def import_2021():
    ...
    file_2021 = FILES_PATH + "resultado-sorteio-2020-2021.pdf"
    ...
    this_year = Results(year=2021, result=[i.serialize for i in query])
...
#     query = app.session.query(Units).order_by(Units.dual).all()
#     this_year = Results(year=2020, result=[i.serialize for i in query])
#     app.session.add(this_year)
...
#     query = app.session.query(Units).order_by(Units.dual).all()
#     this_year = Results(year=2019, result=[i.serialize for i in query])
#     app.session.add(this_year)
...
#     query = app.session.query(Units).order_by(Units.dual).all()
#     this_year = Results(year=2018, result=[i.serialize for i in query])
#     app.session.add(this_year)
